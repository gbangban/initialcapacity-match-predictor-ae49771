...
from sklearn.linear_model import LogisticRegression, PoissonRegressor  # type: ignore
...
class RegressionExperimentsPredictor(Predictor):
    def __init__(self, model: PoissonRegressor, team_encoding: OneHotEncoder) -> None:
        self.model = model
...
def build_model(results: List[Result]) -> Tuple[PoissonRegressor, OneHotEncoder]:
...
def train_experiments_predictor(results: List[Result]) -> Predictor:
    model, team_encoding = build_model(results)

    return RegressionExperimentsPredictor(model, team_encoding)
